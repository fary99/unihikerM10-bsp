...
build_wifibt()
{
	...
	# Use prebuilt ko + firmware from overlay when external/rkwifibt is not present
	if [ ! -d "$RKWIFIBT_DIR" ] || [ ! -d "$RKWIFIBT_DIR/drivers/rtl8723ds" ]; then
		notice "external/rkwifibt missing, using prebuilt WiFi/BT from overlay (no build)."
		mkdir -p "$TARGET_DIR/system/etc/firmware" "$TARGET_DIR/lib/firmware/rtlbt" "$TARGET_DIR/lib/modules"
		if [ -d "$TARGET_DIR/usr/lib/rk3308bs/wifi-firmware" ]; then
			cp -n "$TARGET_DIR/usr/lib/rk3308bs/wifi-firmware"/* "$TARGET_DIR/system/etc/firmware/" 2>/dev/null || true
		fi
		ln -sf system "$TARGET_DIR/vendor"
		if [ -d "$TARGET_DIR/usr/lib/modules" ]; then
			cp -n "$TARGET_DIR/usr/lib/modules"/*.ko "$TARGET_DIR/lib/modules/" 2>/dev/null || true
		fi
		return 0
	fi
	...
	if [[ "$RK_WIFIBT_MODULES" =~ "RTL" ]];then
		...
		if [ -d "$RKWIFIBT_DIR/firmware/realtek/$RK_WIFIBT_MODULES" ]; then
			cp $RKWIFIBT_DIR/firmware/realtek/$RK_WIFIBT_MODULES/* \
				$TARGET_DIR/lib/firmware/rtlbt/ || true
			cp $RKWIFIBT_DIR/firmware/realtek/$RK_WIFIBT_MODULES/* \
				$TARGET_DIR/lib/firmware/ || true
		...
		if compgen -G "$RKWIFIBT_DIR/drivers/$WIFI_KO_DIR/*.ko" > /dev/null; then
			cp $RKWIFIBT_DIR/drivers/$WIFI_KO_DIR/*.ko \
				$TARGET_DIR/lib/modules/ || true
		else
			echo "WARN: no kernel modules found for $RK_WIFIBT_MODULES under drivers/$WIFI_KO_DIR, skip copying .ko"
		fi
