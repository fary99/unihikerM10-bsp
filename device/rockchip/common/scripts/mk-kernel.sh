...
do_build()
{
	...
	case "$1" in
		...
		kernel*)
			...
			# Raw Image + DTB: FAT with boot.scr (no FIT)
			if [ "$RK_BOOT_RAW_IMAGE" = y ] && [ -f "$RK_CHIP_DIR/boot.cmd" ]; then
				run_command "$RK_SCRIPTS_DIR/mk-boot-raw.sh" \
					"$RK_CHIP_DIR/boot.cmd" \
					"$RK_KERNEL_IMG" "$RK_KERNEL_DTB" \
					"kernel/$RK_BOOT_IMG"
			# The FIT image for initrd would be packed in rootfs stage
			elif [ -n "$RK_BOOT_FIT_ITS" ] && \
				...
				run_command "$RK_SCRIPTS_DIR/mk-fitimage.sh" \
					...
					"$RK_KERNEL_IMG" "$RK_KERNEL_DTB" \
					"kernel/resource.img"
				if [ "$RK_BOOT_EXTlinux" = y ]; then
					run_command "$RK_SCRIPTS_DIR/mk-boot-extlinux.sh" \
						"kernel/$RK_BOOT_IMG" \
						"kernel/$RK_BOOT_IMG"
				elif [ "$RK_BOOT_SCR" = y ] && [ -f "$RK_CHIP_DIR/boot.cmd" ]; then
					run_command "$RK_SCRIPTS_DIR/mk-bootscr.sh" \
						"$RK_CHIP_DIR/boot.cmd" \
						"kernel/boot.scr" \
						"kernel/$RK_BOOT_IMG"
				fi
...
init_hook()
{
	...
	if echo $1 | grep -q "^kernel-"; then
		...
		# No explicit version found, default to 6.1
