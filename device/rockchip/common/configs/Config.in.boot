...
config RK_BOOT_SCR
	bool "prepend boot.scr to boot image (FIT after 32K)"
	default n

config RK_BOOT_EXTlinux
	bool "boot partition as FAT with extlinux.conf (distro_bootcmd)"
	default n

config RK_BOOT_RAW_IMAGE
	bool "boot partition as FAT with boot.scr + Image + DTB (no FIT)"
	default n
