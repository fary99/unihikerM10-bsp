...
# For this Unihiker rootfs we use a dedicated usb-rndis-ether systemd service
# and do not rely on the generic usb-gadget/usbdevice framework anymore.
# To avoid long boot delays and repeated usbdevice restarts, completely
# disable this overlay's runtime installation while keeping the files
# in the SDK for reference.
notice "Skipping usb-gadget overlay (using usb-rndis-ether instead)..."
exit 0
